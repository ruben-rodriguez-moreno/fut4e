    ...
    const fetchUserByUsername = async () => {
      ...
      try {
        ...
        if (response.status === 404) {
          ...
          const searchResponse = await fetch(`http://localhost:5000/api/auth/search?username=${encodedUsername}`);
          ...
          if (searchResponse.ok) {
            ...
            if (Array.isArray(searchResults) && searchResults.length > 0) {
              ...
              const videosResponse = await fetch(`http://localhost:5000/api/videos?author=${closestMatch._id}`);

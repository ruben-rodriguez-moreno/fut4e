...
function VideoView({ currentUser, onLike, onComment, onDelete }) {
  ...
  const fetchReportDetails = useCallback(async (reportId) => {
    try {
      const response = await fetch(`https://fut4e.onrender.com/api/support/${reportId}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        ...
        try {
          const healthCheck = await fetch('https://fut4e.onrender.com/api/health');
        ...
        const response = await fetchWithErrorHandling(
          `https://fut4e.onrender.com/api/videos/${videoId}`,
          {}, 
  ...
  const updateReportStatus = async (reportId, status) => {
    const response = await fetch(`https://fut4e.onrender.com/api/support/${reportId}/status`, {
      method: 'PUT',
      headers: {
